/**
 * DART 악재 사전 필터 — 파이프라인 [2단계] ⑦번 필터
 *
 * 매수 전 종목별 DART Open API 리스크 체크:
 *
 * 1. 연속 적자 체크 (fnlttSinglAcnt.json)
 *    - 최근 3기 사업보고서(개별재무제표)에서 당기순이익 조회
 *    - 2기 이상 적자 → 차단 (pass: false)
 *    - 1기 적자 50억 초과 → 경고 (risks에 추가, pass는 유지)
 *
 * 2. 악재 공시 체크 (list.json, 최근 90일)
 *    - B(주요사항), F(감사관련), I(거래소공시) 3개 유형 조회
 *    - 키워드 매칭: 부도/영업정지/감자/상장폐지/횡령/기소/감사의견거절 등
 *    - 1건이라도 매칭 → 차단 (pass: false)
 *
 * 캐시:
 *   - corpCode 매핑 (stock_code → corp_code): 30일 TTL
 *   - 조회 결과: 당일 캐시 (dart-cache.json)
 */
